...
for (file_name in file_names) {
  ...
  # thresholds
  puncta_threshold_p <- 3.6
  rods_threshold_p<- 8
  
  # Clasificacion segun los tresholds en area y perimetro
  mito_data$Classification[ mito_data$Perim. < puncta_threshold_p] <- "Puncta"
  mito_data$Classification[ mito_data$Perim. >= puncta_threshold_p & mito_data$Perim. < rods_threshold_p] <- "Rods"
  mito_data$Classification[ mito_data$Perim. >= rods_threshold_p] <- "Network"
...
combined_results_P <- do.call(rbind, results_list)
# Añadimos la columna llamada Mitos
combined_results_P <- rownames_to_column(combined_results_P, var = "Mitos")

#Agrupamos por experimento y sacamos la media de porcentajes
grouped_results <- combined_results_P %>%
...
# Guardamos en un .csv
write.csv(combined_results_P, "combined_mitochondria_counts_P.csv", row.names = FALSE)
write.csv(grouped_results, "grouped_results_P.csv", row.names = FALSE)

###############################ANOVA##############################

#Construimos un dataset para hacer el analisis
combined_results_P$Mitos[1:15] <- "DMEM"
combined_results_P$Mitos[16:30] <- "KH_2H"
combined_results_P$Mitos[31:45] <- "KH_4H"
combined_results_P$Mitos[46:60] <- "KH+carnitina"
combined_results_P$Mitos[61:75] <- "KH+etomoxir"
combined_results_P$Mitos[76:90] <- "KH+glucosa"
combined_results_P$Mitos[91:105] <- "KH+glutamato"
combined_results_P$Mitos[106:120] <- "KH+oligomicina"
combined_results_P$Mitos[121:135] <- "KH+TBOA+glutamato"
combined_results_P$Mitos[136:150] <- "KH+TBOA"

# Dataset para elanalisis Puncta
anova_dataset_Puncta <- data.frame(
  Mitos = combined_results_P$Mitos,  # Factor
  Puncta = combined_results_P$puncta  # Response variable (puncta counts)
)
...
anova_dataset_Rods <- data.frame(
  Mitos = combined_results_P$Mitos,  # Factor
  Rods = combined_results_P$rods  # Response variable (puncta counts)
)
...
anova_dataset_Network <- data.frame(
  Mitos = combined_results_P$Mitos,  # Factor
  Network = combined_results_P$network  # Response variable (puncta counts)
)
...
#################Plot datos combinados#####################

##Puncta
ggplot(combined_results_P, aes(x = Mitos, y = puncta)) +
...
##Rods
ggplot(combined_results_P, aes(x = Mitos, y = rods)) +
  geom_bar(stat = "identity", fill = "skyblue", width = 0.5) +
  labs(title = "Counts of Mitochondria Morphologies",
       x = "Mitos",
       y = "Counts (rods)") +
...
##Network
ggplot(combined_results_P, aes(x = Mitos, y = network)) +
  geom_bar(stat = "identity", fill = "skyblue", width = 0.5) +
  labs(title = "Counts of Mitochondria Morphologies",
       x = "Mitos",
       y = "Counts (network)") +
